...
tabla_unica <- Base_unica %>% 
  ...
  unique()
View(tabla_unica)
write.xlsx(tabla_unica, "Bases de datos/tabla_unica.xlsx", rowNames = FALSE)
...
La zona metropolitana de ``r unique(Base_zona$NOM_ZM) `` tiene una pobalción de ``r format(sum(Base_zona$POBTOT), big.mark=",") `` de habitantes, según el Censo de Población y Vivienda 2020. La composición de la población se describe en los siguientes puntos:
...
- Densisad poblacional ``r sum(Base_zona$POBTOT)/sum(round(area_km2, 2))``
...
- La estructura de población de la zona metropolitana de ``r unique(Base_zona$NOM_ZM) `` presenta en la siguiente piramide poblacional:
...
##### Indices de envejecimiento, juventud y tasa de dependencia

##### Migración

##### Indice de Desarrollo Humano (IDH), Índice de Marginación (IM) e Índice de Rezago Social (IRS).
