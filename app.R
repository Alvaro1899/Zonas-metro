...
ui <- fluidPage(
  ...
  tags$img(src='logo.png', align = "right", style = "float:right; margin-left:10px; margin-top:10px; height:50px;"),
  ...
  # Agregar un elemento de texto que muestre las instrucciones de uso
...
server <- function(input, output) {
  ...
  output$html <- renderUI({
    ...
    withProgress(message = "Generando reporte", value = 0, {
      ...
      showNotification("Reporte creado, presione en 'Abrir documento'", duration = 15, type = "message") # mostrar la notificación
    })
  })
  ...
  output$html2 <- renderUI({
    ...
    showNotification("¡Listo!",duration = 2, type = "message") # mostrar la notificación
    includeHTML(result)
  })
}
